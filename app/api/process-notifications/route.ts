...
export async function GET(req: Request) {
  ...
  /*
   * ------------------------------------------------------------
   * 2. Claim jobs atomically until the queue is empty or the
   *    per-run batch limit is reached
   * ------------------------------------------------------------
   */
  ...
  while (processedCount < MAX_JOBS_PER_RUN) {
    ...
    if (!job || job.id == null) {
      ...
      return Response.json({
        ...
        message:
          processedCount === 0
            ? "No notification jobs available"
  ...
  return Response.json({
    success: true,
    message: "Notification queue batch limit reached",
    processedCount,
  });
}
